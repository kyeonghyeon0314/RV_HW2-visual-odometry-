...
### 2.3 하드웨어 환경
모든 실험은 **CPU 기반** OpenCV 연산을 통해 수행되었다.
...
각 알고리즘의 처리 성능을 비교한 결과는 아래와 같다.

| Detector | Setting | Total Frames | Total Time (s) | Avg FPS | Note |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Shi-Tomasi** | HighQual | 3983 | 92.76 | 42.94 | Stable, Accurate |
| **Shi-Tomasi** | Balanced | 3983 | 126.83 | 31.40 |  |
| **Shi-Tomasi** | Dense | 3983 | 166.54 | 23.92 |  |
| **FAST** | HighQual | 3983 | 52.48 | 75.89 | **Fastest** |
| **FAST** | Balanced | 3983 | 76.24 | 52.24 |  |
| **FAST** | Dense | 3983 | 138.37 | 28.79 |  |
| **ORB** | HighQual | 3983 | 72.47 | 54.96 | Good Balance |
| **ORB** | Balanced | 3983 | 105.88 | 37.62 |  |
| **ORB** | Dense | 3983 | 124.86 | 31.90 |  |

<br>

**[그림 1] 알고리즘별 FPS 성능 비교**
![FPS Comparison](Result/Report_Assets/comparison_fps.png)

*   **분석:** FAST 알고리즘(HighQual)이 약 76 FPS로 가장 빠른 처리 속도를 보였다. Shi-Tomasi는 정밀한 코너 검출을 수행하므로 상대적으로 연산량이 많아 FPS가 낮게 측정되었다. 특징점 밀도(Dense)가 높아질수록 모든 알고리즘에서 처리 속도가 급격히 저하되는 경향을 보였다.

### 4.2 궤적 시각화 (Trajectory Visualization)

각 실험 설정(Setting)별로 세 가지 알고리즘의 추정 경로를 비교하였다. (Top-Down View, X-Z Plane)

| High Quality Setting | Balanced Setting |
| :---: | :---: |
| ![Traj HighQual](Result/Report_Assets/comparison_traj_HighQual.png) | ![Traj Balanced](Result/Report_Assets/comparison_traj_Balanced.png) |
| **(a) High Quality** | **(b) Balanced** |

<br>

**[그림 2] Dense Setting 및 전체 경로 개요**

| Dense Setting | Overall Overview (Balanced) |
| :---: | :---: |
| ![Traj Dense](Result/Report_Assets/comparison_traj_Dense.png) | ![Traj Overview](Result/Report_Assets/comparison_traj_overview.png) |

*   **Shi-Tomasi (Blue):** 전반적으로 가장 부드럽고(Smooth) 연속적인 궤적을 그린다. 노이즈에 강한 모습을 보인다.
*   **FAST (Red):** 특징점이 매우 많지만 노이즈가 포함될 확률이 높아, 일부 구간에서 궤적이 튀거나 회전각이 과대 추정되는 경향이 관찰된다.
*   **ORB (Green):** Shi-Tomasi와 유사한 궤적을 그리며, 특히 회전 구간에서 강인한 모습을 보였다.
...
본 구현에서는 스케일 팩터(`SCALE`)를 1.0으로 고정하였으므로, 결과 궤적은 **"매 프레임마다 1단위만큼 이동했다"**는 가정하에 그려진 형태(Shape)이다. 실제로는 차가 멈추거나 빠르게 달릴 때 이동 거리가 다르므로, 이 가정은 오차의 원인이 된다.
...
### 5.3 알고리즘 성능 비교 요약
*   **정확도(Accuracy):** Shi-Tomasi $\ge$ ORB $>$ FAST
*   **속도(Speed):** FAST $\gg$ ORB $>$ Shi-Tomasi
*   **결론:**
    *   **실시간성(Real-time)**이 최우선이라면 **FAST**가 유리하다. (단, RANSAC 등으로 이상치 제거 필수)
    *   **정밀도(Precision)**와 궤적의 안정성이 중요하다면 **Shi-Tomasi**가 가장 적합하다.
    *   **ORB**는 속도와 정확도 사이에서 균형 잡힌 성능을 보여주었다.

---

## 6. 결론 (Conclusion)

본 과제를 통해 단안 카메라 기반의 Visual Odometry를 성공적으로 구현하고, 다양한 Feature Detector의 특성을 비교 분석하였다. 특히 해상도 불일치 문제를 파라미터 재계산을 통해 해결함으로써 궤적의 신뢰도를 높일 수 있었다.

실험 결과, Shi-Tomasi 코너 검출기가 가장 안정적인 궤적을 생성함을 확인하였으며, FAST는 빠른 속도에도 불구하고 노이즈에 취약한 특성을 보였다. 단안 VO의 태생적 한계인 스케일 모호성을 확인하였으며, 이를 극복하기 위한 기준 물체(체커보드) 활용 방안을 이론적으로 고찰하였다.
